...
import ru.practicum.event.category.Category;
import ru.practicum.event.category.CategoryRepository;
import ru.practicum.event.event.mapper.CommentMapper;
...
import ru.practicum.event.event.repository.CommentRepository;
import ru.practicum.event.event.repository.EventRepository;
import ru.practicum.event.event.repository.LocationRepository;
...
import ru.practicum.event.event.mapper.EventMapper;
import ru.practicum.exception.ConflictException;
import ru.practicum.exception.NotFoundException;
import ru.practicum.exception.ValidationException;
import ru.practicum.exception.WrongTimeEventException;
...
public class EventServiceImpl implements EventService {
    ...
    public List<EventFullDto> getEventsOfUser(PrivateEventParam param) {
        ...
        List<ViewDto> views = statsClient.getViews(
                LocalDateTime.of(2020, 5, 5, 0, 0, 0),
                LocalDateTime.of(2035, 5, 5, 0, 0, 0),
                uris,
                false);
    ...
    public EventFullDto getEventOfUser(PrivateEventParam param) {
        ...
        EventFullDto event = EventMapper.mapToEventFullDto(checkEvent(eventId, userId));
    ...
    public EventFullDto getEventById(long eventId) {
        return EventMapper.mapToEventFullDto(checkEvent(eventId));
    }
    ...
    public List<ParticipationRequestDto> getRequestsOfUser(PrivateEventParam param) {
        return requestClient.getRequestsOfUser(param.getUserId(), param.getEventId());
    }
    ...
    public EventRequestStatusUpdateResult updateStatusOfRequests(PrivateEventParam param) {
        ...
        for (Long requestId : requestOnUpdateStatus.getRequestIds()) {
            ParticipationRequestDto request = requestClient.getRequestById(requestId);
            ...
            if (event.getConfirmedRequests() >= event.getParticipantLimit()) {
                requestClient.updateRequestStatus(requestId, RequestState.REJECTED);
            ...
            if (requestOnUpdateStatus.getStatus().equalsIgnoreCase("confirmed")) {
                ...
                requestClient.updateRequestStatus(requestId, RequestState.CONFIRMED);
    ...
    public Collection<EventFullDto> getAdminAllEvents(EventFilter filter) {
        ...
        return events.stream()
                .map(event1 -> {
                    ...
                    eventFullDto.setInitiator(userClient.getUserShortById(event1.getInitiatorId()));
    ...
    private Integer countView(Long eventId, LocalDateTime start, LocalDateTime end) {
        ...
        List<ViewDto> views = statsClient.getViews(start, end, uris, true);
        Optional<Integer> countViews = views.stream()
